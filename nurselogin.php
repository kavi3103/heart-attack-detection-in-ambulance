...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  $password = trim($_POST["password"]);
    ...
    if(!empty($username) && !empty($password)){
        ...
        $sql = "SELECT id, username, password FROM nurse WHERE username = ?";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    echo "No account found with that nurse.";
                }
            } else{
  ...
  <body id="login">
    ...
    <center><h1>DETECTOR IN AMBULANCE</h1><br><br>
      <form class="form-signin" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        <h2 class="form-signin-heading">NURSE LOGIN</h2>
        <p>please sign in</p>
        <input type="text" name="username" class="input-block-level" placeholder="username"><br><br>
        <input type="password" class="input-block-level"  name="password" placeholder="Password"><br><br>
        <button class="btn btn-large btn-primary" type="submit">Sign in</button>
      </form></center>
     <a href="nursereg.php">Register</a>
    </div> <!-- /container -->
